...
call "2_unzip_packages.bat"

if %errorlevel% == 0 (
	call "3_setup_packages.bat"
	echo "SUCCESS! Installation is completed"
) else (
	echo "2_unzip_packages.bat failed because 7-zip is not installed in syste. Please unpack all archives in packages internal folders and manually run 3_setup_packages.bat file after"
)
