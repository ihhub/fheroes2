...
if "%~3"=="" (
    echo Syntax: %~n0 VCPKG_ROOT_DIR PLATFORM PACKAGES_DIR
    exit 1
)
...
if not exist "%PACKAGES_DIR%\include"        ( mkdir "%PACKAGES_DIR%\include"        || exit 1 )
if not exist "%PACKAGES_DIR%\lib\%PLATFORM%" ( mkdir "%PACKAGES_DIR%\lib\%PLATFORM%" || exit 1 )
