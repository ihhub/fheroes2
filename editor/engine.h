...
namespace Direction
{
    enum
    {
        ...
        All = TopRow | CenterRow | BottomRow
    };
}
...
namespace Race
{
    enum
    {
        ...
        Random = 0x80,
        Multi = 0x40,
        All = Knight | Barbarian | Sorceress | Warlock | Wizard | Necromancer
    };
...
namespace Building
{
    enum
    {
        ...
        Well = 0x00000008,
        Statue = 0x00000010,
        LeftTurret = 0x00000020,
        RightTurret = 0x00000040,
        Marketplace = 0x00000080,
        ExtraWel2 = 0x00000100 /* Farm, Garbage He, Crystal Gar, Waterfall, Orchard, Skull Pile */,
        Moat = 0x00000200,
        ExtraSpec = 0x00000400 /* Fortification, Coliseum, Rainbow, Dungeon, Library, Storm */,
        Castle = 0x00000800,
        Captain = 0x00001000,
        Shrine = 0x00002000,
        MageGuild1 = 0x00004000,
        MageGuild2 = 0x00008000,
        MageGuild3 = 0x00010000,
        MageGuild4 = 0x00020000,
        MageGuild5 = 0x00040000,
        Dwelling1 = 0x00100000,
        Dwelling2 = 0x00200000,
        Dwelling3 = 0x00400000,
        Dwelling4 = 0x00800000,
        Dwelling5 = 0x01000000,
        Dwelling6 = 0x02000000,
        Upgrade2 = 0x04000000,
        Upgrade3 = 0x08000000,
        Upgrade4 = 0x10000000,
        Upgrade5 = 0x20000000,
...
struct AccessResult
{
    ...
    {}
    AccessResult( int c, bool a, bool f )
        : allowPlayers( c )
        , allowComputer( a )
        , cancelAfterFirstVisit( f )
    {}
...
namespace H2
{
    ...
    public:
        ...
        QByteArray readBlock( size_t, int = -1 );
    ...
    {
        TownPos() {}
        TownPos( const mp2town_t & t, const QPoint & p )
            : QPair<mp2town_t, QPoint>( t, p )
        {}

        const mp2town_t & town( void ) const
        {
            return first;
        }
        const QPoint & pos( void ) const
        {
            return second;
        }
    };
    ...
    {
        HeroPos() {}
        HeroPos( const mp2hero_t & t, const QPoint & p )
            : QPair<mp2hero_t, QPoint>( t, p )
        {}

        const mp2hero_t & hero( void ) const
        {
            return first;
        }
        const QPoint & pos( void ) const
    ...
    {
        SignPos() {}
        SignPos( const mp2sign_t & t, const QPoint & p )
            : QPair<mp2sign_t, QPoint>( t, p )
        {}

        const mp2sign_t & sign( void ) const
        {
            return first;
        }
        const QPoint & pos( void ) const
    ...
    {
        EventPos() {}
        EventPos( const mp2mapevent_t & t, const QPoint & p )
            : QPair<mp2mapevent_t, QPoint>( t, p )
        {}

        const mp2mapevent_t & event( void ) const
        {
            return first;
        }
        const QPoint & pos( void ) const
    ...
    {
        SphinxPos() {}
        SphinxPos( const mp2sphinx_t & t, const QPoint & p )
            : QPair<mp2sphinx_t, QPoint>( t, p )
        {}

        const mp2sphinx_t & sphinx( void ) const
        {
            return first;
        }
        const QPoint & pos( void ) const
...
public:
    ...
    {} /* ground: top left, top, top right, right, bottom right, bottom, bottom left, left, center */
...
struct TypeVariant
{
    ...
    TypeVariant()
        ...
        , variant()
    {}
    TypeVariant( const int & t, const QVariant & v )
        : type( t )
        , variant( v )
    {}
};
...
public:
    MapObject( const QPoint & pos, int uid, int type = MapObj::None )
        : QPoint( pos )
        , objUid( uid )
        , objType( type )
    {}
    ...
    {
        return MapObj::transcribe( objType );
    }
    virtual QString object( void ) const
    {
        return "object";
    }
...
{
    Skill()
        : QPair<int, int>( SkillType::None, SkillLevel::Unknown )
    {}
    Skill( int type, int level )
        : QPair<int, int>( type, level )
    {}
...
struct Troop : public QPair<int, int>
{
    Troop()
        : QPair<int, int>( 0, 0 )
    {}
    Troop( int type, int count )
        : QPair<int, int>( type, count )
    {}
...
struct MapTown : public MapObject
{
    ...
    MapObject * copy( void ) const
    {
        return new MapTown( *this );
    }
...
struct MapResource : public MapObject
{
    ...
    MapResource( const QPoint & pos = QPoint( -1, -1 ), quint32 uid = -1, int res = Resource::Unknown )
        : MapObject( pos, uid, MapObj::Resource )
        , type( res )
        , count( 0 )
    {}
...
struct MapMonster : public MapObject
{
    ...
    MapMonster( const QPoint & pos = QPoint( -1, -1 ), quint32 uid = -1, int mons = Monster::None )
        : MapObject( pos, uid, MapObj::Monster )
        , type( mons )
        , count( 0 )
        , condition( -1 )
    {}
    ...
    MapObject * copy( void ) const
    {
        return new MapMonster( *this );
    }
    void updateInfo( const mp2til_t & );
    int monster( void ) const
    {
        return type;
    }
};
...
struct MapArtifact : public MapObject
{
    ...
    MapArtifact( const QPoint & pos = QPoint( -1, -1 ), quint32 uid = -1, int art = Artifact::None )
        : MapObject( pos, uid, MapObj::Artifact )
        , type( art )
        , spell( Spell::None )
        , condition( -1 )
    {}
...
{
public:
    SharedActionSimple( ActionSimple * ptr )
        : QSharedPointer<ActionSimple>( ptr )
    {}
...
struct ActionSimple
{
    ...
    ActionSimple( int v = MapActions::DefaultAction )
        : type( v )
    {}
...
struct ActionMessage : public ActionSimple
{
    ...
    ActionMessage()
        : ActionSimple( MapActions::Message )
    {}
    ActionMessage( const QString & m )
        : ActionSimple( MapActions::Message )
        , message( m )
    {}
};
...
struct ActionDefault : public ActionSimple
{
    ...
    ActionDefault()
        : ActionSimple( MapActions::DefaultAction )
        , result( true )
    {}
    ActionDefault( const QString & m, bool v )
        : ActionSimple( MapActions::DefaultAction )
        , msg( m )
        , result( v )
    {}
...
struct ActionAccess : public ActionSimple
{
    ...
    ActionAccess()
        : ActionSimple( MapActions::Access )
    {}
    ActionAccess( const QString & m, const AccessResult & v )
        : ActionSimple( MapActions::Access )
        , msg( m )
        , access( v )
    {}
...
struct ActionResources : public ActionSimple
{
    ...
    ActionResources()
        : ActionSimple( MapActions::Resources )
    {}
    ActionResources( const QString & m, const Resources & v )
        : ActionSimple( MapActions::Resources )
        , msg( m )
        , resources( v )
    {}
...
struct ActionArtifact : public ActionSimple
{
    ...
    ActionArtifact()
        : ActionSimple( MapActions::Artifact )
        , artifact( Artifact::None )
        , spell( Spell::None )
    {}
    ActionArtifact( const QString & m, int v )
        : ActionSimple( MapActions::Artifact )
        , msg( m )
        , artifact( v )
        , spell( Spell::None )
...
{
public:
    SharedMapObject( MapObject * ptr )
        : QSharedPointer<MapObject>( ptr )
    {}

    bool operator==( const QPoint & pt ) const
    {
        return data() && pt == data()->pos();
    }
    bool operator==( quint32 uid ) const
    {
...
{
    GameCondition( int cond, const QVariant & val = QVariant() )
        : QPair<int, QVariant>( cond, val )
    {}

    void set( int cond, const QVariant & val = QVariant() )
    {
        first = cond;
        second = val;
    }
...
{
    CondWins()
        : GameCondition( Conditions::Wins )
    {}

    bool allowNormalVictory( void ) const
    {
        return Conditions::AllowNormalVictory & first;
    }
    bool compAlsoWins( void ) const
    {
        return Conditions::CompAlsoWins & first;
    }
    int condition( void ) const
    {
        return Conditions::Wins | index();
    }

    void setAllowNormalVictory( bool f )
    {
        if ( f )
            first |= Conditions::AllowNormalVictory;
        else
            first &= ~Conditions::AllowNormalVictory;
    }
    void setCompAlsoWins( bool f )
    {
        if ( f )
            first |= Conditions::CompAlsoWins;
        else
            first &= ~Conditions::CompAlsoWins;
    }
};
...
{
    CondLoss()
        : GameCondition( Conditions::Loss )
    {}

    int condition( void ) const
    {
        return Conditions::Loss | index();
    }
};
