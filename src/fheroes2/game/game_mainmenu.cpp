...
void Game::mainGameLoop( bool isFirstGameRun, bool isProbablyDemoVersion )
{
    ...
    while ( !exit ) {
        ...
        case fheroes2::GameMode::SELECT_SCENARIO_SIX_HUMAN_PLAYERS: {
            const uint8_t humanPlayCount = static_cast<uint8_t>( static_cast<int>( result ) - static_cast<int>( fheroes2::GameMode::SELECT_SCENARIO_ONE_HUMAN_PLAYER ) );
            // Add +1 since we don't have zero human player option.
            result = Game::SelectScenario( humanPlayCount + 1 );
            break;
        }
