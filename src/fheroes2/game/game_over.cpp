    ...
    if ( isSinglePlayer ) {
        ...
        if ( kingdom.isControlHuman() || isAIAutoControlMode ) {
            ...
            if ( result != GameOver::COND_NONE ) {
                ...
                Video::ShowVideo( { { "LOSE.SMK", Video::VideoControlEnum::PLAY_CUTSCENE_LOOP } } );
            ...
            if ( result != GameOver::COND_NONE ) {
                ...
                AudioManager::ResetAudio();
                Video::ShowVideo( { { "WIN.SMK", Video::VideoControlEnum::PLAY_CUTSCENE_WAIT } }, { standardGameResults() }, true );
