...
void CastleIndexListBox::RedrawBackground( const Point & dst )
{
    ...
    for ( int32_t i = 0; i < middlePartCount; ++i ) {
        fheroes2::Blit( fheroes2::AGG::GetICN( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 1 ), display, dst.x + 2, dst.y + offsetY );
        offsetY += middlePart.height();
    }

    fheroes2::Blit( lowerPart, display, dst.x + 2, dst.y + totalHeight - lowerPart.height() + 45 );

    const fheroes2::Sprite & upperScrollbarArrow = fheroes2::AGG::GetICN( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 3 );
    const fheroes2::Sprite & lowerScrollbarArrow = fheroes2::AGG::GetICN( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 5 );

    totalHeight = rtAreaItems.height + 8 - upperScrollbarArrow.height() - lowerScrollbarArrow.height();

    const fheroes2::Sprite & upperScrollbar = fheroes2::AGG::GetICN( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 7 );
    const fheroes2::Sprite & middleScrollbar = fheroes2::AGG::GetICN( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 8 );
    const fheroes2::Sprite & lowerScrollbar = fheroes2::AGG::GetICN( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 9 );
...
bool ActionSpellTownPortal( Heroes & hero )
{
    ...
    listbox.SetScrollButtonUp( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 3, 4, fheroes2::Point( area.x + 256, area.y + 45 ) );
    listbox.SetScrollButtonDn( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 5, 6, fheroes2::Point( area.x + 256, area.y + 190 ) );
    listbox.SetScrollSplitter( fheroes2::AGG::GetICN( conf.ExtGameEvilInterface() ? ICN::LISTBOX_EVIL : ICN::LISTBOX, 10 ), fheroes2::Rect( area.x + 260, area.y + 68, 14, 120 ) );
    ...
    const int btnICNID = conf.ExtGameEvilInterface() ? ICN::SYSTEME : ICN::SYSTEM;

    btnGroups.createButton( area.x, area.y + 222, btnICNID, 1, 2, Dialog::OK );
    btnGroups.createButton( area.x + 182, area.y + 222, btnICNID, 3, 4, Dialog::CANCEL );
    btnGroups.draw();
