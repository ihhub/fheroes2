...
namespace Battle
{
    void BattlePathfinder::reEvaluateIfNeeded( const Unit & unit )
    {
        ...
        auto currentSettings = std::tie( _pathStart, _speed, _isWide, _isFlying, _isUnitMoatIgnore, _color, _boardStatus );
        const auto newSettings = std::make_tuple( BattleNodeIndex{ unit.GetHeadIndex(), unit.GetTailIndex() }, unit.GetWalkRadius(), unit.isWide(), unit.isFlying(),
                                                  unit.isMoatIgnore(), unit.GetColor(), std::cref( boardStatus ) );
        ...
        for ( size_t nodesToExploreIdx = 0; nodesToExploreIdx < nodesToExplore.size(); ++nodesToExploreIdx ) {
            ...
            if ( _isWide ) {
                ...
                const bool isIgnoreMoat = ( currentHeadCellIdx == pathStartMoatCellIdx || currentTailCellIdx == pathStartMoatCellIdx || _isUnitMoatIgnore );
                const bool isInMoat
                    = isMoatBuilt && !_isUnitMoatIgnore && ( Board::isMoatIndex( currentHeadCellIdx, unit ) || Board::isMoatIndex( currentTailCellIdx, unit ) );
                ...
                const bool isIgnoreMoat = ( currentCellIdx == pathStartMoatCellIdx ) || _isUnitMoatIgnore;
                const bool isInMoat = isMoatBuilt && !_isUnitMoatIgnore && Board::isMoatIndex( currentCellIdx, unit );
                const uint32_t movementPenalty = ( !isIgnoreMoat && isInMoat ) ? MOAT_PENALTY : 1;
