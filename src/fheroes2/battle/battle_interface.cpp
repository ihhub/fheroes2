...
void Battle::Interface::RedrawTroopMoat( const int32_t moatCellId, const Unit & unit )
{
    if ( moatCellId == -1 ) {
        return;
    }
...
void Battle::Interface::RedrawArmies()
{
    ...
    const int32_t moatCellIds[ARENAH] = { 7, 19, 28, 39, 49, 61, 72, 84, 95 };
    ...
    for ( int32_t cellRowId = 0; cellRowId < ARENAH; ++cellRowId ) {
        ...
        if ( castle != nullptr ) {
            ...
            const int32_t moatCellId = castle->isBuild( BUILD_MOAT ) ? moatCellIds[cellRowId] : -1;
