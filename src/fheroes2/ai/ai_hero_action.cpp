...
namespace AI
{
    ...
    void HeroesAction( Heroes & hero, s32 dst_index, bool isDestination )
    {
        ...
        const MP2::MapObjectType objectType = ( dst_index == hero.GetIndex() ? tile.GetObject( false ) : tile.GetObject() );
        bool isAction = true;

        if ( MP2::isActionObject( objectType, hero.isShipMaster() ) )
        ...
        case MP2::OBJ_MONSTER:
            AIToMonster( hero, objectType, dst_index );
            break;
        case MP2::OBJ_HEROES:
        ...
        case MP2::OBJ_ARTIFACT:
            AIToArtifact( hero, objectType, dst_index );
            break;
    ...
    bool HeroesValidObject( const Heroes & hero, s32 index )
    {
        ...
        case MP2::OBJ_BUOY:
            if ( !hero.isObjectTypeVisited( objectType ) && Morale::BLOOD > hero.GetMorale() )
                return true;
            break;

        case MP2::OBJ_MERMAID:
            if ( !hero.isObjectTypeVisited( objectType ) && Luck::IRISH > hero.GetLuck() )
                return true;
            break;
