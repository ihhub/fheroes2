                                                          ...
                                                          const Battle::Units & enemies, bool retreating ) const
{
    ...
    if ( spell.isSingleTarget() ) {
        ...
        const auto areaOfEffectCheck
            = [this, &damageHeuristic, &bestOutcome, &currentUnit, retreating]( const Battle::TargetsInfo & targets, const int32_t index, Color::PlayerColor myColor ) {
                  double spellHeuristic = 0;
...
AI::SpellcastOutcome AI::BattlePlanner::spellSummonValue( const Spell & spell, const Battle::Arena & arena, const Color::PlayerColor heroColor ) const
{
    if ( !spell.isSummon() ) {
        return {};
    }
