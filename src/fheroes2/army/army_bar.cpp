...
namespace
{
    void RedistributeArmy( ArmyTroop & troopFrom, ArmyTroop & troopTarget, Army * armyTarget )
    {
        ...
        if ( overallCount == 1 ) {
            ...
            uint32_t freeSlots = static_cast<uint32_t>( 1 + armyTarget->Size() - armyTarget->GetOccupiedMonsterSlotCount() );
    ...
    void RedistributeTroopToFirstFreeSlot( ArmyTroop & troopFrom, Army * armyTarget, const uint32_t count )
    {
        ...
        const size_t freeSlots = armyTarget->Size() - armyTarget->GetOccupiedMonsterSlotCount();
        if ( freeSlots == 0 )
            return;
