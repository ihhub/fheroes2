...
Funds Kingdom::GetIncome( int type /* INCOME_ALL */ ) const
{
    ...
    if ( INCOME_CAPTURED & type ) {
        ...
        for ( u32 index = 0; resources[index] != Resource::UNKNOWN; ++index )
            resource += ProfitConditions::FromMine( resources[index] ) * world.CountCapturedMines( resources[index], GetColor() );
    }
    ...
    if ( INCOME_ARTIFACTS & type ) {
        ...
        for ( u32 index = 0; artifacts[index] != Artifact::UNKNOWN; ++index )
            for ( KingdomHeroes::const_iterator ith = heroes.begin(); ith != heroes.end(); ++ith )
                resource += ProfitConditions::FromArtifact( artifacts[index] ) * ( **ith ).GetBagArtifacts().Count( Artifact( artifacts[index] ) );

        // TAX_LIEN
        for ( KingdomHeroes::const_iterator ith = heroes.begin(); ith != heroes.end(); ++ith )
            resource -= ProfitConditions::FromArtifact( Artifact::TAX_LIEN ) * ( **ith ).GetBagArtifacts().Count( Artifact( Artifact::TAX_LIEN ) );
    }
