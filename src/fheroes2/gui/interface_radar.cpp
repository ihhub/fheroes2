...
void Interface::Radar::Redraw( void )
{
    ...
    if ( !conf.ExtGameHideInterface() || conf.ShowRadar() ) {
        if ( hide )
            ...
            if ( world.w() != world.h() )
                display.FillRect( area, ColorBlack );
            cursorArea.Hide();
        ...
        for ( s32 xx = 0; xx < world_w; xx += stepx ) {
            ...
            if ( show_tile ) {
                ...
                case MP2::OBJ_HEROES: {
                    ...
                    if ( hero )
                        color = AGG::GetPaletteColor( GetPaletteIndexFromColor( hero->GetColor() ) );
                } break;
                ...
                case MP2::OBJN_CASTLE: {
                    ...
                    if ( castle )
                        color = AGG::GetPaletteColor( GetPaletteIndexFromColor( castle->GetColor() ) );
                } break;
...
void Interface::Radar::RedrawCursor( void )
{
    ...
    if ( !conf.ExtGameHideInterface() || conf.ShowRadar() ) {
        ...
        cursorArea.Move( area.x + offset.x + ( rectMaps.x * areaw ) / world.w(), area.y + offset.y + ( rectMaps.y * areah ) / world.h() );
    }
}
...
void Interface::Radar::QueueEventProcessing( void )
{
    ...
    if ( conf.ShowRadar() && BorderWindow::QueueEventProcessing() ) {
        ...
        if ( le.MouseClickLeft() || le.MousePressLeft() ) {
            ...
            if ( le.MouseWheelUp() ) {
                if ( area.w != world.w() || area.h != world.h() )
                    newSize = Size( world.w(), world.h() );
            }
            else if ( le.MouseWheelDn() ) {
                if ( area.w != RADARWIDTH || area.h != RADARWIDTH )
