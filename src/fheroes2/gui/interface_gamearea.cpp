...
void Interface::GameArea::SetAreaPosition( int32_t x, int32_t y, int32_t w, int32_t h )
{
    ...
    if ( worldSize.width > w ) {
        _minLeftOffset = -( w / 2 ) - Maps::tileWidthPx / 2;
    ...
    if ( worldSize.height > h ) {
        _minTopOffset = -( h / 2 ) - Maps::tileWidthPx / 2;
    ...
    _visibleTileCount = { ( w + Maps::tileWidthPx - 1 ) / Maps::tileWidthPx + 1, ( h + Maps::tileWidthPx - 1 ) / Maps::tileWidthPx + 1 };
...
void Interface::GameArea::renderTileAreaSelect( fheroes2::Image & dst, const int32_t startTile, const int32_t endTile, const bool isActionObject ) const
{
    ...
    const int32_t sizeX = Maps::tileWidthPx + std::abs( startTileOffset.x - endTileOffset.x );
    const int32_t sizeY = Maps::tileWidthPx + std::abs( startTileOffset.y - endTileOffset.y );
...
void Interface::GameArea::QueueEventProcessing( bool isCursorOverGamearea )
{
    ...
    const fheroes2::Point tilePos( ( tileOffset.x / Maps::tileWidthPx ) * Maps::tileWidthPx - _topLeftTileOffset.x + _windowROI.x,
                                   ( tileOffset.y / Maps::tileWidthPx ) * Maps::tileWidthPx - _topLeftTileOffset.y + _windowROI.x );

    const fheroes2::Rect tileROI( tilePos.x, tilePos.y, Maps::tileWidthPx, Maps::tileWidthPx );
...
{
    const int32_t x = ( _topLeftTileOffset.x < 0 ? ( _topLeftTileOffset.x - Maps::tileWidthPx - 1 ) / Maps::tileWidthPx : _topLeftTileOffset.x / Maps::tileWidthPx );
    const int32_t y = ( _topLeftTileOffset.y < 0 ? ( _topLeftTileOffset.y - Maps::tileWidthPx - 1 ) / Maps::tileWidthPx : _topLeftTileOffset.y / Maps::tileWidthPx );
...
{
    SetCenterInPixels( { tile.x * Maps::tileWidthPx + Maps::tileWidthPx / 2, tile.y * Maps::tileWidthPx + Maps::tileWidthPx / 2 } );
}
...
int32_t Interface::GameArea::GetValidTileIdFromPoint( const fheroes2::Point & point ) const
{
    ...
    const int32_t x = offset.x / Maps::tileWidthPx;
    const int32_t y = offset.y / Maps::tileWidthPx;
...
{
    return { tileId.x * Maps::tileWidthPx - _topLeftTileOffset.x + _windowROI.x, tileId.y * Maps::tileWidthPx - _topLeftTileOffset.y + _windowROI.y };
}
