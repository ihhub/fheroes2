        ...
        if ( alphaHero >= 255 && fadeBuilding.IsFadeDone() ) {
            ...
            if ( heroes.Guest() && !readOnly ) {
                ...
                if ( HotKeyPressEvent( Game::HotKeyEvent::MOVE_BOTTOM ) ) {
                    heroes.Guest()->GetArmy().MoveTroops( GetArmy(), keep ? keep->GetID() : Monster::UNKNOWN, true, true );
                    redrawAfterArmyAction = true;
                }
                // Move troops up.
                else if ( HotKeyPressEvent( Game::HotKeyEvent::MOVE_TOP ) ) {
                    GetArmy().MoveTroops( heroes.Guest()->GetArmy(), keep ? keep->GetID() : Monster::UNKNOWN, true, false );
                    redrawAfterArmyAction = true;
                }
            }
