...
void RedrawCurrentInfo( const Point & pos, u32 available, u32 result, const payment_t & paymentMonster, const payment_t & paymentCosts, const Funds & funds,
...
void RedrawResourceInfo( const Surface & sres, const Point & pos, s32 value, s32 px1, s32 py1, s32 px2, s32 py2 )
{
...
void Dialog::DwellingInfo( const Monster & monster, u32 available )
{
    ...
    if ( paymentMonster.crystal ) {
        const Sprite & sres = AGG::GetICN( ICN::RESOURCE, 4 );
        RedrawResourceInfo( sres, pos, paymentMonster.crystal, 225, 75, 240, 103 );
    }
    else
        // mercury
        if ( paymentMonster.mercury ) {
