                          ...
                          const fheroes2::DialogElement * uiElement )
{
    ...
    const int32_t headerHeight = headerText.height( boxAreaWidthPx );
    ...
    headerText.draw( windowArea.x, windowArea.y, boxAreaWidthPx, display );
                          ...
                          const bool englishOnlyVirtualKeyboard )
{
    ...
    const int32_t titleHeight = hasTitle ? titlebox.height( boxAreaWidthPx ) + 10 : 0;
    ...
    const int32_t textboxHeight = textbox.height( boxAreaWidthPx );
    ...
    // Title text.
    if ( hasTitle ) {
        titlebox.draw( frameBoxArea.x, frameBoxArea.y + 12, boxAreaWidthPx, display );
    }

    // Header text.
    textbox.draw( frameBoxArea.x, frameBoxArea.y + 12 + titleHeight, boxAreaWidthPx, display );
...
int Dialog::ArmySplitTroop( const int32_t freeSlots, const int32_t redistributeMax, int32_t & redistributeCount, bool & useFastSplit, const std::string & troopName )
{
    ...
    const int32_t titleHeight = headerHeight + titleText.rows( boxAreaWidthPx ) * titleText.height();
    ...
    const int32_t bodyHeight = slotSeparationText.rows( boxAreaWidthPx ) * slotSeparationText.height();
    ...
    header.draw( pos.x, pos.y + 2, boxAreaWidthPx, display );
    titleText.draw( pos.x, pos.y + 2 + headerHeight, boxAreaWidthPx, display );
    ...
    if ( freeSlots > 1 ) {
        ...
        slotSeparationText.draw( pos.x, pos.y + textTopOffset + titleHeight + 37, boxAreaWidthPx, display );
