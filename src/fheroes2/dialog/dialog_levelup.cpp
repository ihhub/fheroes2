...
int DialogSelectSecondary( const std::string & name, const int primarySkillType, const Skill::Secondary & sec1, const Skill::Secondary & sec2, Heroes & hero )
{
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.MouseClickLeft( button_learn2.area() ) || Game::HotKeyPressEvent( Game::DEFAULT_RIGHT ) ) {
            return sec2.Skill();
        }

        if ( le.MouseClickLeft( button_hero.area() ) || Game::HotKeyPressEvent( Game::DEFAULT_READY ) ) {
            LocalEvent::GetClean();
