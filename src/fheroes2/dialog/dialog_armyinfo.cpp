...
int Dialog::ArmyInfo( const Troop & troop, int flags, bool isReflected )
{
    ...
    while ( le.HandleEvents() ) {
        if ( flags & BUTTONS ) {
            ...
            // upgrade
            if ( buttonUpgrade.isEnabled() && ( le.MouseClickLeft( buttonUpgrade.area() ) || Game::HotKeyPressEvent( Game::UPGRADE_TROOP ) ) ) {
            ...
            // dismiss
            if ( buttonDismiss.isEnabled() && ( le.MouseClickLeft( buttonDismiss.area() ) || Game::HotKeyPressEvent( Game::DISMISS_TROOP ) )
                 && Dialog::YES
