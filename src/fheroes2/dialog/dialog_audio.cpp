...
namespace
{
    void drawDialog( const std::vector<fheroes2::Rect> & rects )
    {
        ...
        // Music volume.
        const fheroes2::Sprite & musicVolumeIcon = fheroes2::AGG::GetICN( ICN::SPANEL, ( Audio::isValid() && conf.MusicVolume() > 0 ) ? 1 : 0 );
        std::string value;
        if ( Audio::isValid() && conf.MusicVolume() > 0 ) {
        ...
        // Sound volume.
        const fheroes2::Sprite & soundVolumeOption = fheroes2::AGG::GetICN( ICN::SPANEL, ( Audio::isValid() && conf.SoundVolume() > 0 ) ? 3 : 2 );
        if ( Audio::isValid() && conf.SoundVolume() > 0 ) {
