...
bool ScreenSwitch::QueueEventProcessing( void )
{
    ...
    if ( le.MouseClickLeft( rtScreen1 ) )
        result = SCREEN1;
    else if ( castle.isBuild( BUILD_CASTLE ) && le.MouseClickLeft( rtScreen2 ) )
        result = SCREEN2;
    else if ( castle.isBuild( BUILD_CASTLE ) && le.MouseClickLeft( rtScreen3 ) )
        result = SCREEN3;
    else if ( castle.isBuild( BUILD_CASTLE ) && le.MouseClickLeft( rtScreen4 ) )
        result = SCREEN4;
    else if ( castle.GetLevelMageGuild() && le.MouseClickLeft( rtScreen5 ) )
        result = SCREEN5;
    else if ( le.MouseClickLeft( rtScreen6 ) )
        result = SCREEN6;
...
screen_t CastleOpenDialog1( Castle & castle, bool readonly )
{
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( !readonly && screenSwitch.QueueEventProcessing() && SCREEN1 != screenSwitch.result )
            ...
            // exit
            if ( le.MouseClickLeft( rectExit ) || Game::HotKeyPressEvent( Game::EVENT_DEFAULT_EXIT ) )
            break;
        else if ( !readonly && buttonNext.isEnable() && le.MouseClickLeft( buttonNext ) )
            return SCREENOUT_NEXT;
        else if ( !readonly && buttonPrev.isEnable() && le.MouseClickLeft( buttonPrev ) )
            return SCREENOUT_PREV;
        ...
        if ( heroes.Guest() && selectArmy2.isValid() ) {
            ...
            if ( le.MouseCursor( selectArmy1.GetArea() ) && selectArmy1.QueueEventProcessing() ) {
                cursor.Hide();
                RedrawResourceBar( Point( dst_rt.x + 4, dst_rt.y + 176 ), kingdom.GetFunds() );
...
screen_t CastleOpenDialog6( Castle & castle, bool readonly )
{
    ...
    if ( castle.isBuild( BUILD_CASTLE ) ) {
        ...
        if ( hero1 )
            hero1->PortraitRedraw( rectRecruit1.x + 4, rectRecruit1.y + 4, PORT_MEDIUM, display );

        crest.Blit( rectRecruit2 );
        if ( hero2 )
            hero2->PortraitRedraw( rectRecruit2.x + 4, rectRecruit2.y + 4, PORT_MEDIUM, display );
    ...
    while ( le.HandleEvents() ) {
        if ( !readonly && screenSwitch.QueueEventProcessing() && SCREEN6 != screenSwitch.result )
            ...
            if ( le.MouseClickLeft( rectExit ) || Game::HotKeyPressEvent( Game::EVENT_DEFAULT_EXIT ) )
            break;
        else if ( hero1 && le.MouseClickLeft( rectRecruit1 ) && Dialog::OK == castle.DialogBuyHero( hero1 ) ) {
            castle.RecruitHero( hero1 );
            return SCREEN1;
        }
        else if ( hero2 && le.MouseClickLeft( rectRecruit2 ) && Dialog::OK == castle.DialogBuyHero( hero2 ) ) {
            ...
            // show marketplace
            if ( castle.isBuild( BUILD_MARKETPLACE ) && le.MouseClickLeft( rt1 ) )
            Dialog::Marketplace();
        else
            // buy boat
            if ( castle.isBuild( BUILD_SHIPYARD ) && le.MouseClickLeft( rt3 ) && Dialog::OK == Dialog::BuyBoat( castle.AllowBuyBoat() ) )
            castle.BuyBoat();
        else
            // show thieves guild
...
void RedrawTownSprite( const Rect & rt, const Castle & castle )
{
    ...
    if ( !castle.AllowBuild() )
        AGG::GetICN( ICN::LOCATORS, 24 ).Blit( rt.x + 43, rt.y + 5 );
}
