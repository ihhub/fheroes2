...
int PocketPC::MainMenu( void )
{
    ...
    while ( le.HandleEvents() ) {
        ...
        else if ( Game::HotKeyPressEvent( Game::EVENT_BUTTON_CREDITS ) || le.MouseClickLeft( rectCredits ) )
            return Game::CREDITS;
        else if ( Game::HotKeyPressEvent( Game::EVENT_BUTTON_HIGHSCORES ) || le.MouseClickLeft( rectHighScores ) )
            return Game::HIGHSCORES;
        else if ( Game::HotKeyPressEvent( Game::EVENT_DEFAULT_EXIT ) || le.MouseClickLeft( rectQuitGame ) )
...
int PocketPC::NewGame( void )
{
    ...
    // mainmenu loop
    while ( le.HandleEvents() ) {
        if ( Game::HotKeyPressEvent( Game::EVENT_BUTTON_STANDARD ) || le.MouseClickLeft( rectStandardGame ) )
            return Game::NEWSTANDARD;
        else if ( Game::HotKeyPressEvent( Game::EVENT_BUTTON_CAMPAIN ) || le.MouseClickLeft( rectCampaignGame ) )
            return Game::MAINMENU;
        else if ( Game::HotKeyPressEvent( Game::EVENT_BUTTON_MULTI ) || le.MouseClickLeft( rectMultiGame ) )
            return Game::NEWMULTI;
        else if ( Game::HotKeyPressEvent( Game::EVENT_DEFAULT_EXIT ) || le.MouseClickLeft( rectCancel ) )
            return Game::MAINMENU;
    }
...
int PocketPC::NewMulti( void )
{
    ...
    // mainmenu loop
    while ( le.HandleEvents() ) {
        if ( Game::HotKeyPressEvent( Game::EVENT_BUTTON_HOTSEAT ) || le.MouseClickLeft( rectHotSeat ) )
            return Game::NEWHOTSEAT;
        else if ( Game::HotKeyPressEvent( Game::EVENT_BUTTON_NETWORK ) || le.MouseClickLeft( rectNetwork ) ) {
