...
void Maps::Tiles::updatePassability()
{
    ...
    if ( !isActionObject && ( _mainAddon._objectIcnType != MP2::OBJ_ICN_TYPE_UNKNOWN ) && _mainAddon._imageIndex < 255 && ( ( _mainAddon._layerType >> 1 ) & 1 ) == 0 && !isShadow() ) {
...
StreamBase & Maps::operator<<( StreamBase & msg, const Tiles & tile )
{
    ...
    return msg << tile._index << tile._terrainImageIndex << tile._terrainFlags << tile._tilePassabilityDirections << tile._mainAddon._uid
               << static_cast<ObjectIcnTypeUnderlyingType>( tile._mainAddon._objectIcnType ) << tile._mainAddon._imageIndex << static_cast<MainObjectTypeUnderlyingType>( tile._mainObjectType )
...
StreamBase & Maps::operator>>( StreamBase & msg, Tiles & tile )
{
    ...
    msg >> tile._tilePassabilityDirections >> tile._mainAddon._uid;
    ...
    if ( Game::GetVersionOfCurrentSaveFile() < FORMAT_VERSION_1001_RELEASE ) {
        ...
        tile._mainAddon._objectIcnType = static_cast<MP2::ObjectIcnType>( objectIcnType >> 2 );
    }
    else {
        ObjectIcnTypeUnderlyingType objectIcnType = MP2::OBJ_ICN_TYPE_UNKNOWN;
        msg >> objectIcnType;
    ...
    msg >> tile._occupantHeroId >> tile._isTileMarkedAsRoad >> tile._addonBottomLayer >> tile._addonTopLayer >> tile._mainAddon._layerType;
