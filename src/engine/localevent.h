...
public:
    ...
    static void setWindowMovedEventHook( std::function<void( const int, const int )> hook )
    {
        LocalEvent::_onWindowMoved = std::move( hook );
    }
...
private:
    ...
    inline static std::function<void( const int, const int )> _onWindowMoved;
