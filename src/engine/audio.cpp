...
namespace
{
    struct AudioSpec
    {
        ...
        // access to the audio. The audio can only be accessed (and feeded) from the main thread. The chunk size
        // should be large enough to avoid playback issues caused by other activities performed on the main thread.
        // On the other hand, it shouldn't be too big, because it is possible to stop the audio playback only after
        // the current chunk has been completely played.
        int chunkSize = 8192;
#else
        int chunkSize = 2048;
#endif
