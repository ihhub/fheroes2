...
class MapReader(val input: InputStream?) {
    companion object {
        ...
        fun readMap(input: InputStream?): MapHeader {
            ...
            val isPoL = isPriceOfLoyalty(tilesBuffer.asUByteArray())

            return MapHeader(readMapName(headerBuffer), width, height, isPoL)
        }

        private val polMapObjectIndexes = listOf(
            247.toUByte(), // MP2::OBJ_BARRIER
            249.toUByte(), // MP2::OBJ_EXPANSION_DWELLING
            250.toUByte(), // MP2::OBJ_EXPANSION_OBJECT
            251.toUByte(), // MP2::OBJ_JAIL
            248.toUByte(), // MP2::OBJ_TRAVELLER_TENT
        )

        private fun isPoLTile(tilesBuffer: UByteArray, tileIndex: Int): Boolean {
        ...
        private fun isPriceOfLoyalty(tilesBuffer: UByteArray): Boolean {
            var tileIndex = 0;
